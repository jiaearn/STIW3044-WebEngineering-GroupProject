...
if (isset($_POST['addfav'])){
    ...
    if(empty($email)){
            echo "<script type='text/javascript'>alert('Please login to perform this function!');window.location.assign('rankIndicators.php');</script>'";
    } else{
        $sqlsearch = "SELECT * FROM `tbl_favourite` WHERE email = '$email' &&  `universityID`='$universityID'";
        $result = $conn->query($sqlsearch);
        
        if ($result->num_rows > 0) {
            echo "<script type='text/javascript'>alert('This record already add into favourite list!');window.location.assign('rankIndicators.php');</script>'";
        }else{
            $addFavourite = "INSERT INTO `tbl_favourite`(`universityID`, `email`) VALUES('$universityID','$email')";
            if(mysqli_query($conn, $addFavourite)){
            echo  "<script type='text/javascript'>alert('Success!');window.location.assign('rankIndicators.php');</script>'";
            }
            else{
                echo "<script type='text/javascript'>alert('Failed!!');window.location.assign('rankIndicators.php');</script>'";
            }
        }
    }
}
...
if (isset($_POST['addcompare'])){
    if(empty($email)){
        ...
        if ($result->num_rows > 0) {
            echo "<script type='text/javascript'>alert('This record already add into compare list!');window.location.assign('rankIndicators.php');</script>'";
        }else{
            $sqlcompare = "SELECT * FROM `tbl_compare` WHERE email = '$email'";
            $result = $conn->query($sqlcompare);
            if ($result->num_rows >= 2){
                echo  "<script type='text/javascript'>alert('You can only add maximum 2 universities in the compare list');window.location.assign('rankIndicators.php');</script>'";
            }else{
...
<script src="https://kit.fontawesome.com/56e1a6fc52.js" crossorigin="anonymous"></script>
	...
	<title>Ranking Indicators</title>
...
<div class="header" style="position:relative"><img src="../images/logo.jpg" alt="Asia Top U Logo">
  ...
  <img class="mySlides" src="../images/sNUSbanner.png" style="width:100%">
	...
	<a href="uniRanking.php"><div class="universitybtn">
